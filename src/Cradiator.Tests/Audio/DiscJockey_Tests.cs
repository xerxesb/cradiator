...
namespace Cradiator.Tests.Audio
{
	...
	public class DiscJockey_Tests
	{
		...
		const string BrokenWav = "broken.wav";
		const string FixedWav = "fixed.wav";
		...
		public void SetUp()
		{
			...
			{
				BrokenBuildSound = BrokenWav,
				FixedBuildSound = FixedWav,
			    PlaySounds = true
			};
		...
		public void CanPlay_BuildStatus_If_SuccessThenFailure()
		{
			...
			_audioPlayer.AssertWasNotCalled(a => a.Play(BrokenWav));
			
			_discJockey.PlaySounds(_projectList1Failure);
			_audioPlayer.AssertWasCalled(a => a.Play(BrokenWav));
		}
		...
		public void CanPlay_BuildStatus_If_SuccessThenFail_WithMultipleStatus()
		{
			...
			_audioPlayer.AssertWasNotCalled(a => a.Play(BrokenWav), a=>a.Repeat.Once());
		
			_discJockey.PlaySounds(_projectList2BothFailure);
			_audioPlayer.AssertWasCalled(a => a.Play(BrokenWav), a=>a.Repeat.Once());
		}
		...
		public void CanPlaySound_ThenUpdateConfig_ThenNotPlaySound()
		{
            ...
            _discJockey.ConfigUpdated(new ConfigSettings { PlaySounds = true, BrokenBuildSound = BrokenWav});
		...
		{
			_discJockey.PlaySounds(_projectList1Failure);
			_audioPlayer.AssertWasNotCalled(a => a.Play(FixedWav));

			_discJockey.PlaySounds(_projectList1Success);
			_audioPlayer.AssertWasCalled(a => a.Play(FixedWav));
		}
